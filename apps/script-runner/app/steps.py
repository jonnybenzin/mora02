"""The pipeline step vocabulary: one handler per verb, and the endpoint a
compiled flow curls.

Split out of main.py in September 2026 — it was 1800 of that file's 3400 lines
and shares nothing with the media service above it but the module they sat in.
The dispatch table at the bottom is the contract: a name in the vocabulary must
have a handler here, and the drift check in tests/pipeline holds the two
against each other.

Same shape as agents.py, mcp_tools.py and speech.py: a router main.py includes.
"""
...
@router.get("/pipeline/vocab-stats")
