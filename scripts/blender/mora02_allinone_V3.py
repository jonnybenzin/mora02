...
def get_logo_center():
    """Mittelpunkt des Logos in World-Space (nach +0.5 Cube-Offset)."""
    tw = get_total_width()
    return (tw / 2.0, 2.5, 0.5)
...
def build_logo(theme="dark"):
    ...
    for idx, (wx, wy, wz) in enumerate(homes):
        bpy.ops.mesh.primitive_cube_add(size=1.0, location=(wx + 0.5 - cx, wy + 0.5 - cy, 0.5 - cz))
        ...
        obj["cube_idx"] = idx
        obj["home_x"] = float(wx + 0.5 - cx)
        obj["home_y"] = float(wy + 0.5 - cy)
        obj["home_z"] = 0.5 - cz
    ...
    if not bpy.data.objects.get("MORA02_Sun"):
        ...
        bpy.data.objects["MORA02_Sun"].location = (0, 0, 20)
...
def factory_pos_at_frame(idx, frame, total_frames):
    ...
    home = homes[idx]
    ocx, ocy, ocz = get_logo_center()
    hp = (home[0]+0.5-ocx, home[1]+0.5-ocy, 0.5-ocz)
    ...
    if frame <= 0:
        return hp, (0, 0, 0)
    target = targets[idx]
    tp = (target[0]+0.5-ocx, target[1]+0.5-ocy, 0.5-ocz)
    ...
    for i, (sf, ef, sp, ep) in enumerate(moves):
        s = (sp[0]+0.5-ocx, sp[1]+0.5-ocy, sp[2]+0.5-ocz)
        e = (ep[0]+0.5-ocx, ep[1]+0.5-ocy, ep[2]+0.5-ocz)
...
def _mora02_frame_handler_inner(scene):
    ...
    if props.anim_mode == 'waves':
        ...
        for obj in objects:
            ...
            if progress >= 1.0:
                obj.scale = (1, 1, 1)
                obj.location = (obj.get("home_x",0), obj.get("home_y",0), obj.get("home_z",0))
                for ms in obj.material_slots:
                    if ms.material and ms.material.use_nodes:
